...
contract BuktiAttestationTest is Test {
    ...
    function test_zkvmBatchEncodingDecodesExactly() public {
        bytes memory pv =
            hex"";
        ...
        // v3 completeness commitment is carried through the same encoding.
        assertEq(a1.numSwaps, 5);
        assertEq(a1.swapsRoot, hex"28dd7839b9f3fb63eb34dc5f3b306a39d46db8e190f164a9a50079b992533f16");
        ...
        assertEq(a2.numSwaps, 5);
        assertEq(a2.swapsRoot, hex"38494c39df5ada37240d1584475d1dc90bc9463b345bc2fe72ac236801e4bca9");
    }
    ...
    function test_completenessCommitmentStoredAndRead() public {
        BuktiOutput memory o = _out(AGENT, 630);
        bytes32 expectedRoot = o.swapsRoot;
        BuktiOutput[] memory outs = new BuktiOutput[](1);
        outs[0] = o;
        attest.submitBatchAttestation(abi.encode(outs), "");

        (bytes32 root, uint32 numSwaps, bool exists) = attest.getCompleteness(AGENT);
        assertTrue(exists);
        assertEq(root, expectedRoot);
        assertEq(numSwaps, 11);
        // also surfaced in the full attestation tuple
        BuktiAttestation.Attestation memory a = attest.getAttestation(AGENT);
        assertEq(a.swapsRoot, expectedRoot);
        assertEq(a.numSwaps, 11);
    }

    function test_completenessUnknownWalletIsZero() public view {
        (bytes32 root, uint32 numSwaps, bool exists) = attest.getCompleteness(address(0xdead));
    ...
    function test_largeBatchGasIsBounded() public {
        ...
        // v3 adds the completeness commitment (swapsRoot = one extra 32-byte slot), so the
        // per-wallet cost rises ~22k vs the metrics-only v2 — still trivially under a block.
        assertLt(used / N, 160_000); // per-wallet cost bounded
