...
> **Status:** isolated R&D module. The verification core (accumulator + VAA parse, guardian
> secp256k1 quorum, Merkle inclusion, price decode) is implemented and unit-tested against a
> **real** Hermes update. Folding it into the live batch circuit (behind SP1's secp256k1 +
> keccak precompiles) and re-proving is the scoped next step — kept out of `lib`/`program`
> for now so new crypto deps can never change the **deployed** batch vkey.
...
2. **Verify the VAA** — double-keccak the body, `ecrecover` each guardian signature, require a
   **13-of-19 quorum** of *distinct, in-order* guardians from set #4. (Guardian addresses read
   from the Wormhole core contract `getGuardianSet(4)` on Ethereum and pinned in `GUARDIAN_SET_4`.)
